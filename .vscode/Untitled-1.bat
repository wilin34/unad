<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Restaurante El Buen Paladar</title>
</head>
<body>
    <h1>Restaurante El Buen Paladar</h1>
    <h2>Donde comer es un placer... ven y disfruta nuestros ricos potajes</h2>
    <p>En <strong>El Buen Paladar</strong> queremos que disfrutes… por eso siéntate, relájate y empieza a disfrutar… tu comida se va a preparar al momento tenlo por seguro!!!!!</p> 
    <p>Elige el plato que disfrutarás el día de hoy</p>
    <table>
        <tr>
            <td><img src="images/arroz-con-pollo.jpg" alt="arroz con pollo"></td>
            <td><img src="images/cau-cau.jpg" alt="cau cau"></td>
            <td><img src="images/ceviche.jpg" alt="ceviche"></td>
        </tr>
        <tr>
            <td><img src="images/chaufa.jpg" alt="chaufa"></td>
            <td><img src="images/huancaina.jpg" alt="papa a la huancaina"></td>
            <td><img src="images/lomo-saltado.jpg" alt="lomo saltado"></td>
        </tr>
    </table>
    <p>Visítanos en Calle Los Geranios Nº 777, San Vicente de Cañete</p>
</body>
</html>